...
def test_iter_rave(structure, randomize_ties):
    alg = RAVE(Node(0, 0), equivalence_param=10, structure=structure, iter_stop="iters",
...
def test_cpu_time_rave(structure, randomize_ties):
    alg = RAVE(Node(0, 0), equivalence_param=10, structure=structure, iter_stop="cpu_time",
                randomize_ties=randomize_ties, action_space_size=2)
